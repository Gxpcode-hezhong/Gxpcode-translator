...
def backfill_continuations(elements: list) -> int:
    ...
    empty = [
        ...
        if not el.get("zh") and el.get("label") == "para"
    ]
    empty.sort()
    ...
    for idx in empty:
        ...
        if parent is not None and elements[parent].get("zh"):
            elements[idx]["zh"] = elements[parent].get("zh", "")
            count += 1
...
def build_translated_json(elements_path: str, pages_dir: str, output_path: str):
    ...
    for el in output:
        if el.get("label") in ("tab", "table"):
            en_tr = (el.get("en") or "").count("<tr>")
            zh_tr = (el.get("zh") or "").count("<tr>")
            if en_tr != zh_tr:
                row_issues.append({"index": el.get("index", 0), "page": el.get("page", 0), "en_rows": en_tr, "zh_rows": zh_tr})
