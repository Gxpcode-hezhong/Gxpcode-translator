...
Usage:
  python step5b_export_md.py --elements recognition.json --translated translated.json --out-dir output/ --title "ISPE Guide"
"""
...
def build_markdown(data: dict, translated_json: dict, title: str) -> str:
    ...
    for i, el in enumerate(elements):
        ...
        if is_heading(label):
            # Map section levels to markdown heading depth
            prefix = {"sub_sub_sec": "####", "sub_sec": "###"}.get(label, "##")
            lines.append(f"{prefix} {en}")
            if zh:
                lines.append(f"**{zh}**")
            lines.append("")
        else:
