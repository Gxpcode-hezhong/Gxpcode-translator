...
Usage:
  python step5a_export_html.py --elements recognition.json --translated translated.json --out-dir output/ --title "ISPE Guide"
"""
...
def fix_image_paths(text: str, paddleocr_dir: str, html_out_dir: str) -> str:
    ...
    # Try known locations for figures directory (PaddleOCR version may vary)
    candidates = [
        os.path.join(paddleocr_dir, "markdown", "figures"),
        os.path.join(paddleocr_dir, "markdown", "images"),
    ]
    figures_dir = next((d for d in candidates if os.path.isdir(d)), "")
    if not figures_dir:
        # Fallback: search recursively under paddleocr_dir
        for root, dirs, _ in os.walk(paddleocr_dir):
            if any(d in ("figures", "images") for d in dirs):
                figures_dir = os.path.join(root, "figures") if "figures" in dirs else os.path.join(root, "images")
                if os.path.isdir(figures_dir):
                    break
    if not figures_dir:
        return text  # No images directory found, skip silently
...
def flatten_elements(data: dict) -> list:
    ...
    for page in data.get("pages", []):
        pn = page.get("page_number", page.get("page", 0))
...
""" + "\n".join(f"    <li>{idx}</li>" for idx in empty_indices) + """
  ...
  <p style="font-size:0.8em; margin-top:8px;">修复: 重新运行 step4_merge.py。</p>
</div>"""
