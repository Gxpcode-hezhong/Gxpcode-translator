...
SKILL_DIR = Path(__file__).resolve().parent.parent.parent  # Navigate: lib → scripts → skill root
...
def _is_word_boundary(text, start, end):
    """Check that match [start:end] is at word boundaries in text.
    A word boundary means: char before start is NOT a letter (or start==0),
    AND char after end is NOT a letter (or end==len(text)-1).
    This prevents short abbreviations (IR, ND, CE) from matching as substrings
    inside ordinary English words (requirements, standalone, devices)."""
...
def find_matches(text, automaton, term_set, cn_map):
    ...
    for end_idx, (idx, en, cn) in automaton.iter(lower_text):
        ...
        # Filter: only keep hits at word boundaries
        if _is_word_boundary(lower_text, start, end_idx):
