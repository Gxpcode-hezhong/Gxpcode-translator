...
the full text from the elements JSON via step1b_rebuild_page.py and overwrites the
...
def main():
    ...
    p.add_argument("--build-page-script", default=None,
                   help="Path to step1b_rebuild_page.py (auto-detected if not given)")
    args = p.parse_args()
    ...
    # Auto-detect step1b_rebuild_page.py
    if args.build_page_script:
        build_script = Path(args.build_page_script)
    else:
        skill_dir = Path(__file__).resolve().parent.parent
        build_script = skill_dir / "scripts" / "step1b_rebuild_page.py"

    if not build_script.exists():
        print(f"ERROR: step1b_rebuild_page.py not found at {build_script}")
        sys.exit(1)
    ...
    for pf in page_files:
        ...
        if result.returncode != 0 or not temp_out.exists():
            print(f"   ❌ step1b_rebuild_page.py failed for page {page_num}: {result.stderr[:200]}")
            continue
